...
function Init(rr) {
  ...
  if (FenString == StandardFen) {
    ...
    EnPass = -1;
    HalfMove[0] = 0;
    console.log('INIT this.HalfMove: ', HalfMove);
...
function MoveForward(nn, rr) {
  ...
  console.log(`ShortPgnMoveText[0][${CurVar}]: `, ShortPgnMoveText[0][CurVar]);
...
function StoreMove(ii, ttype1, xx1, yy1, jj, ttype3, xx3, yy3, sstore) {
  console.log('storemove: HalfMove: ', HalfMove);
  var iis_check = 0,
    ll,
    cc = MoveCount - StartMove,
      ...
      if (tt == "B" || tt == "C") {
        ...
        if (nn >= 0 && nn <= 63) {
          ...
          if (tt == "B")
            ...
            document.images[ImageOffset + nn].style.backgroundColor = HEX2RGBA(
              cc
            );
        }
      }
...
function SetBoardClicked(nn) {
  ...
  if (BoardClicked >= 0) {
    if (BoardClicked < 64) {
      if (isRotated)
        document.images[
          ImageOffset + 63 - BoardClicked
        ].style.borderColor = BorderColor;
      else
        document.images[
          ImageOffset + BoardClicked
        ].style.borderColor = BorderColor;
    } else
      document.images[
        ImageOffset + BoardClicked + 3
      ].style.borderColor = BorderColor;
  }
  BoardClicked = nn;
  if (BoardClicked >= 0) {
...
function SetPgnMoveText(ss, vvariant, rroot, sstart) {
  ...
  console.log('ShortPgnMoveText', ShortPgnMoveText);
}
...
function GetFEN(sshort) {
  ...
  console.log(`HalfMove: `, HalfMove);
  console.log(`HalfMove[${this.MoveCount} - ${this.StartMove}]: `, HalfMove[this.MoveCount - this.StartMove]);
  ...
  for (ii = jj; ii < ll && activeAnchor < 0; ii++) {
    if (TargetDocument.anchors[ii].name == nn) {
      ...
      TargetDocument.anchors[
        activeAnchor
      ].style.backgroundColor = activeAnchorBG;
      if (
